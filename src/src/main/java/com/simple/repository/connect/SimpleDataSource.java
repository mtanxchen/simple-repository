...
public class SimpleDataSource {
    ...
    public synchronized SimpleConnection activeConnect() throws SQLException, ClassNotFoundException {
        ...
        if (MIN_CONNECT_POOL_SIZE > CONNECTION_POOL.size()) {
            initConnectPool(DEFAULT_CONNECT_POOL_SIZE);
        }
        // 过期则重新获取
        if (connection.overdue() || connection.getConnection().isClosed()) {
    ...
    public void giveBack(SimpleConnection connection) throws SQLException {
        // 超时或大于连接池或关闭事务提交的连接直接关闭
        if (connection.overdue() || MAX_CONNECT_POOL_SIZE < CONNECTION_POOL.size() || !connection.getConnection().getAutoCommit()) {
            connection.close();
            return;
        }
     ...
     * 短连接数
     */
    public static Integer DEFAULT_CONNECT_POOL_SIZE = 64;


    /**
     * 最大连接数
     */
    public static Integer MAX_CONNECT_POOL_SIZE = 128;

    /**
     * 最小连接数
     */
    public static Integer MIN_CONNECT_POOL_SIZE = 32;
